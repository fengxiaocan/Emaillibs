...
public class EmailUtil {
    ...
    public static void sendMail2Other(String title,String mailContent,String receiveMailAccount)
    {
        try {
            ...
            // 使用smtp：简单邮件传输协议
            props.put("mail.smtp.host",myEmailSMTPHost);//存储发送邮件服务器的信息
            ...
            message.setRecipient(Message.RecipientType.TO,new InternetAddress(receiveMailAccount));//设置收件人,并设置其接收类型为TO
    ...
    ) throws Exception
    {
        ...
        props.setProperty("mail.transport.protocol", "smtp");   // 使用的协议（JavaMail规范要求）
        props.setProperty("mail.smtp.host", smtpAddr);   // 发件人的邮箱的 SMTP 服务器地址
        props.setProperty("mail.smtp.auth", "true");            // 需要请求认证
    ...
    ) throws Exception
    {
        ...
        mc.addMailcap(
                "text/plain;; x-java-content-handler=com.sun.com.evil.mail.handlers.text_plain");
        mc.addMailcap(
                "multipart/*;; x-java-content-handler=com.sun.com.evil.mail.handlers.multipart_mixed");
        mc.addMailcap(
                "message/rfc822;; x-java-content-handler=com.sun.com.evil.mail.handlers.message_rfc822");
